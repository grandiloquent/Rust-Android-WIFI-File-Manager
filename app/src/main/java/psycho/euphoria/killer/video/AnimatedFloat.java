...
public class AnimatedFloat {
    ...
    public float set(float mustBe, boolean force) {
        ...
        if (transition) {
            ...
            final float t = AndroidUtilities.clamp((now - transitionStart - transitionDelay) / (float) transitionDuration, 0, 1);
    ...
    public float getTransitionProgress() {
        ...
        final long now = SystemClock.elapsedRealtime();
        return AndroidUtilities.clamp((now - transitionStart - transitionDelay) / (float) transitionDuration, 0, 1);
    }
