package psycho.euphoria.killer.video;// https://raw.githubusercontent.com/DrKLO/Telegram/master/TMessagesProj/src/main/java/org/telegram/ui/Components/VideoPlayerSeekBar.java
...
public class VideoPlayerSeekBar {
    ...
    public void updateTimestamps(CharSequence text, long videoDuration) {
        if (text == null || videoDuration < 0) {
            clearTimestamps();
            return;
        }
        ...
        for (int i = 0; i < links.length; ++i) {
            ...
            if (link != null && link.getURL() != null && link.label != null && link.getURL().startsWith("video?")) {
                Integer seconds = AndroidUtilities.parseInt(link.getURL().substring(6));
    ...
    public void draw(Canvas canvas, View view) {
        ...
        if (loopBack > 0) {
            ...
            setPaintColor(AndroidUtilities.blendARGB(progressColor, smallLineColor, transitionProgress), 1f);
            drawProgressBar(canvas, rect, paint);
            rect.left = wasLeft;
            setPaintColor(AndroidUtilities.blendARGB(circleColor, getProgress() == 0 ? Color.TRANSPARENT : smallLineColor, transitionProgress), 1f - transitionProgress);
    ...
    private void setPaintColor(int color, float alpha) {
        if (alpha < 1f) {
            color = AndroidUtilities.setAlphaComponent(color, (int) (Color.alpha(color) * alpha));
        }
